...
class ScopeBasedParamProvider:
    def __init__(self, sum_params, leaf_params, scope_name=None):
...
class BernoulliVector(NodeVector):
    ...
    def modes_tf(self, case_num=0):
        probs = 1 / (1 + tf.exp(-self.params))
        return probs

    def probs_of_modes(self, case_num=0):
        probs = 1 / (1 + np.exp(-self.np_params[case_num]))
        return probs
    ...
    def reconstruct(self, max_idxs, node_num, case_num, sample):
        if sample:
            ...
            #my_sample = self.probs_of_modes(case_num)
    ...
    # return tf graph
    def reconstruct_tf(self, max_idxs, node_num, case_num, sample):
        if sample:
            my_sample = self.dist.sample()
        else:
            my_sample = self.modes_tf()  # instead of self.np_means[case_num]

        scope = np.reshape(self.scope, [len(self.scope), -1]).astype(np.int32)
        full_sample = tf.scatter_nd(tf.constant(scope),
                                    tf.transpose(my_sample, perm=[1, 2, 0]),
                                    tf.constant([self.num_dims, self.size, 2708]))
        full_sample = tf.transpose(full_sample, perm=[2, 0, 1])
...
class GaussVector(NodeVector):
    ...
    def reconstruct_tf(self, max_idxs, node_num, case_num, sample):
        if sample:
            ...
            my_sample = self.means  # instead of self.np_means[case_num]
...
class PoissonVector(NodeVector):
    def __init__(self, region, args, name,
                 given_means=None, given_stddevs=None, mean=0.0, num_dims=0):
        super().__init__(name)
        self.local_size = len(region)
        ...
        self.dist = dists.Poisson(self.means)
    ...
    def reconstruct_tf(self, max_idxs, node_num, case_num, sample):
        if sample:
            my_sample = self.means #self.dist.sample()
        else:
            my_sample = self.means  # instead of self.np_means[case_num]
...
class ProductVector(NodeVector):
    ...
    def sample(self, inputs, seed=None):
        # print("Sample from inputs in Product Vector")
        # If you sess run on anything, the next sess run will be different because sample
    ...
    def reconstruct_tf(self, max_idxs, node_num, case_num, sample):
        result1 = self.vector1.reconstruct_tf(max_idxs, None, case_num, sample)
        result2 = self.vector2.reconstruct_tf(max_idxs, None, case_num, sample)

        result = tf.expand_dims(result1, -1) + tf.expand_dims(result2, -2)
        return tf.reshape(result, [result1.shape[0], -1, self.size])
        #return result1 + result2
...
class SumVector(NodeVector):
    ...
    def forward(self, inputs):
        ...
        if self.args.linear_sum_weights:
            ...
            self.sample_child_idx_relaxed = dists.RelaxedOneHotCategorical(0.5, logits=tf.transpose(child_values, perm=[0, 2, 1])).sample()   # todo is this temperature (0.5) ok?
    ...
    def reconstruct_tf(self, max_idx_tensors_relaxed, node_num, case_num, sample):
        inputs_reconstruction = []
        for i, inp_vector in enumerate(self.inputs):
            inputs_reconstruction.append(inp_vector.reconstruct_tf(max_idx_tensors_relaxed, None, case_num, sample))  # max_idx_tensors_relaxed[inp_vector] *

        if sample:
            sample_sum = dists.RelaxedOneHotCategorical(0.01, logits=self.weights).sample()
            inputs_reconstruction = tf.cast(tf.expand_dims(tf.concat(inputs_reconstruction, 2), axis=-1), tf.float32) + \
                                    tf.log(tf.expand_dims(tf.clip_by_value(sample_sum, 1e-32, 1), 1))
        else:
            # (this should not be used)
            sample_sum = dists.RelaxedOneHotCategorical(0.1, logits=self.weights).sample()
            inputs_reconstruction = tf.cast(tf.expand_dims(tf.concat(inputs_reconstruction, 2), axis=-1), tf.float32) + tf.log(tf.expand_dims(sample_sum, 1))

        inputs_reconstruction = tf.reduce_logsumexp(inputs_reconstruction, axis=2)
    ...
    def sample(self, inputs, seed=None, differentiable=False):
        ...
        # reformulate so it is differentiable
        dist = dists.Categorical(logits=logits)
        dist = dists.RelaxedOneHotCategorical(1, logits=logits)
        sampled = dist.sample([inputs.shape[0]], seed=seed)
        print(sampled)
        interpoloat_sampled = tf.matmul(logits, sampled)  # interpolate between of the childrens samples

        # Num weights is arbitrarely chosen...
        # print(dist)
        # just batch size or number of samples, how you wanna see it
        indices = dist.sample([inputs.shape[0]], seed=seed)
        # why is it shape (num_samples, num_sums)
        # this means I will choose one of the 9 distr
        # print(inputs.shape[0])
        # print(indices)

        # 10, 20, filled with one row same val (20 times) 10 classes
...
class RatSpn(object):
    ...
    def _make_spn_from_region_graph(self):
        ...
        for i, leaf_region in enumerate(rg_layers[0]):
            ...
            elif self.args.dist == 'Poisson':
                name = 'poisson_{}'.format(i)
                leaf_vector = PoissonVector(leaf_region, self.args, name, mean=self.default_mean,
    ...
    # return tf graph instead of np values
